...
|팀원|느낀점|
|-----|---|
|김혜진|K(Keep) : 하루에 두 번 이상 회의를 함으로써 백-프론트 간 서로 진행사항을 공유할 수 있어서 좋았다. MyBatis 대신 JPA를 처음 써보았는데 처음엔 어려웠지만 기존의 방식보다 훨씬 편하고 구현도 간단하다고 느꼈고, 다음에도 JPA를 사용하여 프로젝트를 기획하고 싶다. P(Problem) : 초기에 자동배포를 하지 않아서 서버를 초기화하는 등 배포에 대한 여러움이 있었다. 또한 프로젝트를 하기 전에 시간에 쫓기지 않기 위해서 나름 구체적으로 계획을 세웠는데 계획한 대로 진행되지 않았다. T(Try) : 일정에 차질이 생기고 문제가 발생하더라도 팀원들끼리 의기투합하여 하나하나 해결해나갔다.다음 프로젝트를 할 때에는 자동배포에 대해 학습의 필요성을 느꼈다.|
|정지원|내용|
|최 강|내용|
