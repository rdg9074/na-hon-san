...
public interface DMRepository extends JpaRepository<DMEntity, String> {
    ...
    @Query(value = "select d from DMEntity d where d.fromUserId=:userId group by d.toUserId")
    List<DMEntity> findByfromUserIdGrouptoUserId(UserEntity userId);

    @Query(value = "select d from DMEntity d where d.toUserId=:userId group by d.fromUserId")
    List<DMEntity> findBytoUserIdGroupFromUserId(UserEntity userId);
